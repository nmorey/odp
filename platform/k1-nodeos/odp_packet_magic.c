...
static int _magic_scall_init(void){
	return __k1_syscall0(MAGIC_SCALL_INIT);
}
static int _magic_scall_open(int id){
	return __k1_syscall1(MAGIC_SCALL_OPEN, id);
}
static int _magic_scall_get_mac(int id, void * mac){
	return __k1_syscall2(MAGIC_SCALL_GETMAC, id, (uint32_t)mac);
}
static int _magic_scall_recv(int id, void* packet){
	return __k1_syscall2(MAGIC_SCALL_RECV, id, (uint32_t)packet);
}
static int _magic_scall_send(int id, void * buf, unsigned len){
	return __k1_syscall3(MAGIC_SCALL_SEND, id, (uint32_t)buf, len);
}
...
{
	int ret = _magic_scall_open(if_id);
	if(ret)
		return ret;
...
void magic_get_mac(const pkt_magic_t *const pkt_magic, void * mac_addr){
	_magic_scall_get_mac(pkt_magic->id, mac_addr);
}
int magic_recv(pkt_magic_t *const pkt_magic, odp_packet_t pkt_table[], int len)
{
	...
	for (i = 0; i < len; i++) {
		...
		recv_bytes = _magic_scall_recv(pkt_magic->id, pkt_buf);
...
int magic_send(pkt_magic_t *const pkt_magic, odp_packet_t pkt_table[], unsigned len)
{
	...
	while (i < len) {
		...
		ret = _magic_scall_send(pkt_magic->id, frame, frame_len);
