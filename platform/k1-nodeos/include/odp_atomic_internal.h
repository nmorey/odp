...

{
	...
	do {
		uint32_t old_val = LOAD_U32(atom->_type);
		int ret = __k1_compare_and_swap((unsigned*)&atom->_type, old_val, val);
		if(ret)
			return old_val;
	} while(1);
#endif
		...
		_odp_memmodel_t failure ODP_UNUSED)
{
	...
	*exp = oldval;
	return 0;
#elif defined(__K1B__)
	return __k1_compare_and_swap((unsigned*)&atom->_type, *exp, val);
#endif
}
