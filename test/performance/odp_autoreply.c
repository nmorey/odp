...
static inline odp_queue_t lookup_dest_q(odp_packet_t pkt)
{
	...
	return odp_pktio_outq_getdef(pktio_src);
}
...
static int drop_err_pkts(odp_packet_t pkt_tbl[], unsigned len)
{
	...
	for (i = 0, j = 0; i < len; ++i) {
		...
		/* Switch L2 source and dest */
		odph_ethhdr_t *eth = (odph_ethhdr_t *)(buf + odp_packet_l2_offset(pkt));
		odph_ethaddr_t tmpmac;
		memcpy(&tmpmac, eth->src.addr, ODPH_ETHADDR_LEN);
		memcpy(eth->src.addr, eth->dst.addr, ODPH_ETHADDR_LEN);
		memcpy(eth->dst.addr, &tmpmac, ODPH_ETHADDR_LEN);

		unsigned l3_offset = odp_packet_l3_offset(pkt);
		if (l3_offset != ODP_PACKET_OFFSET_INVALID) {
			/* If there is an L3, switch src and dest here too */
			odph_ipv4hdr_t *ip = (odph_ipv4hdr_t *)(buf + l3_offset);
...
{
	printf("\n"
	       "OpenDataPlane L2 loopback application.\n"
	       "\n"
	       "Usage: %s OPTIONS\n"
	       "  E.g. %s -i eth0,eth1 -m 0 -t 1\n"
	       " In the above example,\n"
	       " eth0 will send pkts to eth0\n"
	       " eth1 will send pkts to eth1\n"
	       "\n"
	       "Mandatory OPTIONS:\n"
