...
float convert_throughput(float avg_cycles,
...
void send_data(uint32_t interface,
...
void sync_initiator(uint32_t interface, uint32_t mailbox,
					uint32_t initiator_cluster_id __attribute__ ((unused)),
...
void sync_client(uint32_t interface, uint32_t mailbox,
				 ...
				 uint32_t client_cluster_id __attribute__ ((unused)),
	...
	for (header_cnt = 0, packet_group = 0; packet_group < nb_group_packet; packet_group++) {
		...
		for (idx_in_group = 0; idx_in_group < nb_packets_per_group;
			...
			mppa_dnoc[0]->tx_ports[tx_chan].push_data_eot.reg =
			    (header_cnt << 32) | data_cnt;
			__k1_cpu_backoff(cycles_to_wait);
		}
