...
int mppa_88E1111_copper_get_loopback_status(mppa_88E1111_interface_t* interface, uint8_t *loopback_enabled)
{
  int status = 0;
  uint16_t reg;
...
int mppa_88E1111_fiber_get_loopback_status(mppa_88E1111_interface_t* interface, uint8_t *loopback_enabled)
{
  int status = 0;
  uint16_t reg;
...
int mppa_88E1111_line_get_loopback_status(mppa_88E1111_interface_t* interface, uint8_t *loopback_enabled)
{
  int status = 0;
  uint16_t reg;
...
int mppa_88E1111_check_phy_identifier(mppa_88E1111_interface_t* interface)
{
  ...
  if(phy_identifier != 0x01410cc2 || status != 0) {
    status = -1;
#ifdef VERBOSE
    printf("[88E1111 0x%.2x] => Identifier is not the expected one.\n", interface->chip_id);
...
int mppa_88E1111_open(__mppa_platform_type_t platform, mppa_88E1111_interface_list_t* interface_list)
{
  int i;
  ...
  case __MPPA_ETH_FPGA:
#ifdef VERBOSE
    printf("[88E1111] Opening a 88E1111 chip MDIO interface for MPPA Ethernet FPGA board.\n");
#endif
    ...
    interface_list->interface[0].context = (void*)(&(interface_list->interface[0].mdio_master));
    interface_list->interface[0].mppa_88E1111_read  = (int (*)(volatile void*, uint8_t, uint8_t, uint16_t*))mppa_eth_mdio_read;
    interface_list->interface[0].mppa_88E1111_write = (int (*)(volatile void*, uint8_t, uint8_t, uint16_t))mppa_eth_mdio_write;
    return mppa_eth_mdio_init((int)(&(interface_list->interface[0].mdio_master)));
    break;
  case __MPPA_AB01:
#ifdef VERBOSE
    printf("[88E1111] Opening two 88E1111 chip I2C interface for MPPA Ethernet AB01 board.\n");
#endif
    ...
    for(i=0;i<2;i++) {
      interface_list->interface[i].context = (void*)(interface_list->interface[i].i2c_master);
      interface_list->interface[i].mppa_88E1111_read  = (int (*)(volatile void*, uint8_t, uint8_t, uint16_t*))mppa_i2c_register_read;
      interface_list->interface[i].mppa_88E1111_write = (int (*)(volatile void*, uint8_t, uint8_t, uint16_t))mppa_i2c_register_write;
      if(mppa_i2c_init(interface_list->interface[i].i2c_master, interface_list->interface[i].i2c_bus ,interface_list->interface[i].i2c_coma_pin, interface_list->interface[i].i2c_reset_n_pin, interface_list->interface[i].i2c_int_n_pin, interface_list->interface[i].i2c_gic) != 0) return -1;
    }
    break;
...
int mppa_88E1111_close(mppa_88E1111_interface_list_t* interface_list)
{
  ...
  for(i=0;i<interface_list->interface_nb;i++) {
    ...
    if(interface_list->interface[i].i2c_master == 0) {
      if(mppa_eth_mdio_finish((int)&(interface_list->interface[i].mdio_master)) != 0) status = -1;
    }
  }
  return status;
}
...
int mppa_88E1111_dump_register(mppa_88E1111_interface_t* interface)
{
  ...
  for(j=0;j<(sizeof(page_reg)/sizeof(uint32_t));j++) {
	...
	/* Select Pages*/
	if(i<30) {
	  status |= interface->mppa_88E1111_write(interface->context, interface->chip_id, 22, j & 0x7F);
	} else {
	  status |= interface->mppa_88E1111_write(interface->context, interface->chip_id, 29, j & 0x7F);
	}
	if(status != 0) {
#ifdef VERBOSE
	  printf("[88E1111 0x%.2x] 88E1111 register dump failed.\n", interface->chip_id);
#endif
	  return -1;
	}
	status |= interface->mppa_88E1111_read(interface->context, interface->chip_id, i, &val);
	if(status != 0) {
#ifdef VERBOSE
	  printf("[88E1111 0x%.2x] 88E1111 register dump failed.\n", interface->chip_id);
#endif
...
#ifdef VERBOSE
	printf("[88E1111 0x%.2x] Register %d: 0x%.4x\n", interface->chip_id, i, val);
#endif
      }
    }
  }
...
int mppa_88E1111_get_led_mode(mppa_88E1111_interface_t* interface, uint16_t* mode)
{
  ...
  *mode = reg & 0x0fff;
  ...
  reg = (*mode >> 10) & 0x0003;
  if(reg == 0) printf("auto.\n");
  if(reg == 1) printf("blink.\n");
  if(reg == 2) printf("on.\n");
  if(reg == 3) printf("off.\n");
...
int mppa_88E1111_fiber_get_rate(mppa_88E1111_interface_t* interface, uint8_t* rate)
{
  ...
  case 2:
    ...
    break;
  default:
    printf("[88E1111 0x%.2x] Fiber rate is set to an unexpected value 0x%.2x.\n", interface->chip_id, *rate);
    break;
  }
#endif
  return status;
...
int mppa_88E1111_copper_get_rate(mppa_88E1111_interface_t* interface, uint8_t* rate)
{
  ...
  *rate = ((reg >> 5) & 0x2) | ((reg >> 13) & 0x1);
  ...
  case 2:
    ...
    break;
  default:
    printf("[88E1111 0x%.2x] Copper rate is set to an unexpected value 0x%.2x.\n", interface->chip_id, *rate);
    break;
  }
#endif
  return status;
...
int mppa_88E1111_configure(mppa_88E1111_interface_t* interface)
{
  ...
  if(mppa_88E1111_copper_autoneg_enable(interface) != 0) return -1;
  return 0;
}
...
int mppa_88E1111_configure_all(mppa_88E1111_interface_list_t* interface_list)
{
  ...
  for(i=0;i<interface_list->interface_nb;i++) {
    if(mppa_88E1111_configure(&(interface_list->interface[i])) != 0) return -1;
  }
  return 0;
}
...
int mppa_88E1111_synchronize_all(mppa_88E1111_interface_list_t* interface_list)
{
  ...
  for(i=0;i<interface_list->interface_nb;i++) {
    if(mppa_88E1111_synchronize(&(interface_list->interface[i])) != 0) return -1;
  }
  return 0;
}
...
int mppa_88E1111_get_packet_generator_status(mppa_88E1111_interface_t* interface, uint8_t* stat)
{
  ...
  *stat = (((uint8_t)reg) >> 2) & 0x0f;
  ...
  if((*stat & 0x04) == 0) {
    printf("[88E1111 0x%.2x] Packet generator is using random data.\n", interface->chip_id);
  } else {
    printf("[88E1111 0x%.2x] Packet generator is using 0x5a data.\n", interface->chip_id);
  }
  if((*stat & 0x02) == 0) {
    printf("[88E1111 0x%.2x] Packet generator length : 64 bytes.\n", interface->chip_id);
  } else {
    printf("[88E1111 0x%.2x] Packet generator length : 1518 bytes.\n", interface->chip_id);
  }
  if((*stat & 0x01) == 0) {
    printf("[88E1111 0x%.2x] Packet generator error insertion is disabled.\n", interface->chip_id);
  } else {
    printf("[88E1111 0x%.2x] Packet generator error insertion is enabled.\n", interface->chip_id);
  }
#endif
  return status;
}
...
int mppa_88E1111_copper_autoneg_complete(mppa_88E1111_interface_t* interface, uint8_t* autoneg_done)
{
...
int mppa_88E1111_copper_get_autoneg_ability(mppa_88E1111_interface_t* interface, uint8_t* autoneg_en)
{
...
int mppa_88E1111_copper_get_energy_detect_mode(mppa_88E1111_interface_t* interface, uint8_t* mode)
{
  ...
  *mode = (uint8_t)((reg >> 8) & 0x0003);
  ...
  switch(*mode) {
  case 2:
    printf("[88E1111 0x%.2x] Energy detect is set to \"Sense only on receive\".\n", interface->chip_id);
    break;
  case 3:
    printf("[88E1111 0x%.2x] Energy detect is set to \"Sense and perdiodically transmit NLP\".\n", interface->chip_id);
    break;
  default:
...
int mppa_88E1111_copper_set_energy_detect_mode(mppa_88E1111_interface_t* interface, uint8_t mode)
{
  ...
  switch(mode) {
  case 2:
    printf("[88E1111 0x%.2x] Setting energy detect is to \"Sense only on receive\".\n", interface->chip_id);
    break;
  case 3:
    printf("[88E1111 0x%.2x] Setting energy detect to \"Sense and perdiodically transmit NLP\".\n", interface->chip_id);
    break;
  default:
...
int mppa_88E1111_fiber_get_energy_detect_status(mppa_88E1111_interface_t* interface, uint8_t* energy_detected)
{
...
int mppa_88E1111_copper_get_energy_detect_status(mppa_88E1111_interface_t* interface, uint8_t* sleep)
{
...
int mppa_88E1111_virtual_cable_tester(mppa_88E1111_interface_t* interface, uint8_t* lane_status)
{
  ...
  *lane_status = 0;
  for(lane_id=0;lane_id<4;lane_id++) {
#ifdef VERBOSE
    printf("[88E1111 0x%.2x] Performing VCT test on lane[%u].\n", interface->chip_id, lane_id);
#endif
    // Select page "i"
    status |= interface->mppa_88E1111_read(interface->context, interface->chip_id, 22, &reg);
    ...
    status |= interface->mppa_88E1111_write(interface->context, interface->chip_id, 22, reg);
    // Enable VCT test
    status |= interface->mppa_88E1111_read(interface->context, interface->chip_id, 28, &reg);
    reg |= 0x8000;
    status |= interface->mppa_88E1111_write(interface->context, interface->chip_id, 28, reg);
    // Wait for some time....
